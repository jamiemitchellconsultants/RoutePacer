...
{
    private readonly GpsSpikeFilter filter = new(); private RouteTrack? route; private RideSummary? ride; private DateTimeOffset started; private DateTimeOffset? pausedAt; private TimeSpan pausedTotal; private GeoFix? previousFix; private double totalDistance; private int? previousSegment; private long sequence;
    ...
    public async Task StartAsync(Guid routeId)
    {
        ...
        route = await routes.GetAsync(routeId) ?? throw new InvalidOperationException("Route not found."); State = RideSessionState.Starting; started = clock.GetUtcNow(); pausedTotal = TimeSpan.Zero; pausedAt = null; previousFix = null; totalDistance = 0; sequence = 0;
    ...
    public async Task StopAsync() { if (State is not (RideSessionState.Running or RideSessionState.Paused)) throw new InvalidOperationException("Ride is not active."); State = RideSessionState.Stopping; await location.StopAsync(); await wakeLock.ReleaseAsync(); await FinalizeAsync(RideStatus.Completed); }
    ...
    private async Task OnFixAsync(GeoFix fix)
    {
        ...
        var match = new RouteMatcher().Match(route, fix, previousSegment); if (match is null) return; previousSegment = match.SegmentIndex; if (previousFix is not null) totalDistance += GeoMath.HaversineMeters(previousFix.Latitude, previousFix.Longitude, fix.Latitude, fix.Longitude); previousFix = fix; var pacing = new PacingService().Calculate(route, match, started, fix);
    ...
    private Task OnLocationErrorAsync(LocationFailure failure) => FinalizeAsync(RideStatus.Interrupted);
    ...
    private void Publish(PacingSnapshot? pacing) { if (route is not null) { Snapshot = new TrackerSnapshot(State, route.Summary, pacing, pacing?.Match.RouteDistanceMeters ?? 0); SnapshotChanged?.Invoke(Snapshot); } }
}

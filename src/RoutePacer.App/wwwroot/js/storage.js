...
export const deleteRoute = routeId => withTransaction(["routes", "route_points"], "readwrite", tx => { tx.objectStore("routes").delete(routeId); const index = tx.objectStore("route_points").index("routeId"); index.openCursor(routeId).onsuccess = e => { const cursor = e.target.result; if (cursor) { cursor.delete(); cursor.continue(); } }; });
...
export const deleteRide = rideId => withTransaction(["rides", "ride_points"], "readwrite", tx => { tx.objectStore("rides").delete(rideId); const index = tx.objectStore("ride_points").index("rideId"); index.openCursor(rideId).onsuccess = e => { const cursor = e.target.result; if (cursor) { cursor.delete(); cursor.continue(); } }; });
